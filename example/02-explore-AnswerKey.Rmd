...
But, had I not aligned in two steps, the cowplot grid would have looked like 
...
mockdata %>% 
  ...
  adorn_ns() %>%
  knitr::kable(caption = 'ECOG performance score by Sex',
...
**CHALLENGE**: Style this table for your report

```{r}
library(arsenal)

tableby(ph.ecog ~ sex + age + ph.karno, data = mockdata) %>% 
  summary()
```
...
Add a `fig.path` in your knitr opts in the setup chunk and `knit` again !

**HINT**: https://stackoverflow.com/questions/58598873/r-notebook-markdown-does-not-save-chunk-plots-using-fig-path-chunk-option 
